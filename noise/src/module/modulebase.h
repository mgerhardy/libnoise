  ...
  namespace module
    {
      ...
      ///
      /// A <i>noise module</i> (often shortened to <i>module</i>) outputs a value
      /// by generating coherent noise or by calculating mathematical functions.
      /// An application can use these noise values in the following ways:
      ...
      /// complex noise functions.  A noise module used as a source of values for
      /// another noise module is called a <i>source module</i>.  Each of these
      /// source modules may also have their own source modules, and so on.
      ///
      /// There is no limit to the number of noise modules that can be connected
      /// together in this way.  However, each connected module increases the time
      /// required to calculate a value.
      ///
      ...
      ///
      /// A generator module outputs a value generated by a "solid
      /// noise" function (such as Perlin noise) or by a mathematical
      /// calculation.
      ///
      ...
      /// - Max: Outputs the larger of the two values from two source
      ///   modules.
      ///
      ...
      ///   blender module.
      /// - Select: Outputs the value selected from one of two source
      ///   modules chosen by a selector module.
      ///
      /// <i>Transformer Modules</i>
      ///
      /// A transformer module applies a transformation to the input point before
      /// retrieving the value from the source module.  A transformer module does
      ...
      ///
      /// Each noise module requires the attachment of a certain
      /// number of source modules before an application can retrieve
      /// a value.  For example, the Add module requires two source
      /// modules, while the Perlin module requires none.  Call a
      /// noise module's GetSourceModuleCount() method to retrieve the
      /// number of source modules required by that module.
      ///
      /// For non-selector modules, it does not matter which index
      /// value an application assigns to a particular source module,
      /// but for selector modules, a source module's purpose is
      /// defined by its index value.  For example, consider the
      /// Select noise module, which requires three source modules.
      /// The control module is the source module assigned an index
      /// value of @b 2.  The control module determines whether the
      /// noise module will output the value from the source module
      /// assigned an index value of @b 0 or the value from the source
      /// module assigned an index value of @b 1.
      ///
      /// <b>Retrieving Values from a Noise Module</b>
      ///
      /// Once an application has connected all required source
      /// modules to a noise module, the application can now begin to
      /// retrieve values from that module.
      ///
      /// To retrieve a value at a specific point, pass the ( @a x, @a
      /// y, @a z ) coordinates of that point to the GetValue()
      /// method.  This point is known as the <i>input point</i>.
      ///
      /// <b>Using a Noise Module to Generate Terrain Heightmaps or Textures</b>
      ///
      /// One way to generate a terrain heightmap or a texture is to
      /// first allocate a 2-dimensional array of floating-point
      /// values.  For each array element, pass the array subscripts
      /// as @a x and @a y coordinates to the GetValue() method
      /// (leaving the @a z coordinate set to zero) and place the
      /// returned value into the array element.
      ///
      /// <b>Creating Your Own Noise Modules</b>
      ///
      /// Create a class that publicly derives from Module.
      ///
      /// In the constructor, call the base class' constructor,
      /// passing the return value from GetSourceModuleCount() to that
      /// constructor.
      ///
      /// Override the GetSourceModuleCount() method.  From this
      /// method, return the number of source modules required by your
      /// module.
      ///
      /// Override the GetValue() pure virtual method.  For generator
      /// modules, generate the noise value and return that value.
      /// For other modules, retrieve the value from each source
      /// module stored in the protected @a m_pSourceModule array,
      /// mathematically combine those values, and return the combined
      /// value.
      ///
      /// When developing a noise module, you must ensure that your
      /// noise module does not modify any source module or control
      /// module connected to it; a noise module can only modify the
      /// value from those source modules.  You must also ensure that
      /// if an application fails to connect all required source
      /// modules via the SetSourceModule() method and then attempts
      /// to call the GetValue() method, your module will raise an
      /// assertion.
      ///
      /// It shouldn't be too difficult to create your own noise
      /// module.  If you still have some problems, take a look at the
      /// source code for Add, which is a very simple noise
      /// module.
      class Module
	{
	...
	public:
	  ...
	  ///
	  /// @pre The index value is between @b 0 and one less than the number of
	  /// source modules required by this noise module, inclusive.
	  ...
	  /// source modules before an application can call the GetValue() method.
	  ...
	  /// index values assigned to any other source module added to this noise
	  /// module.  If an index value has already been assigned to a source
	  /// module, this noise module replaces the old source module with the
	  /// new source module.
	  ///
	  /// Before an application can call the GetValue() method, it must first
	  /// attach all required source modules.  To determine the number of
	  /// source modules required by this noise module, call the
	...
	private:
	  ...
	  /// This restriction is necessary because if this object was copied, all
	  /// source modules assigned to this noise module would need to be copied
	  /// as well.
