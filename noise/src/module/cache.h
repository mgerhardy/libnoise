...
namespace noise
{
  ...
  namespace module
  {
    ...
    /// ( @a x, @a y, @a z ) coordinates of the input point, are stored (cached)
    /// in this noise module.
    ///
    /// If the application passes an input point to the GetValue() method that
    /// is equal to the previously passed input point, this module returns the
    /// cached value without having the source module recalculate the output
    /// value.
    ///
    ...
    /// the source module will redundantly calculate the same output value once
    /// for each module in which it is included.
    ///
